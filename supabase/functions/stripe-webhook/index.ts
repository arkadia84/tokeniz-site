...
// Demo helpers (same as complete-formation)
...
Deno.serve(async (req) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // ── Banking white-glove payment ──────────────────
    if (meta.product === "banking-white-glove") {
      await supabase
      ...
      // TODO: trigger banking confirmation email when template is ready
      logStep("Banking payment confirmed", { application_id: meta.application_id });
    ...
    if (!userId || !companyName || !seriesNumber || !plan) {
      logStep("Missing metadata", { meta });
      return new Response(JSON.stringify({ received: true, message: "Missing metadata" }), { status: 200 });
    }
    ...
    if (existing) {
      logStep("Company already exists, skipping", { id: existing.id });
      return new Response(JSON.stringify({ received: true, already_existed: true }), { status: 200 });
    }

    try {
      // Mint NFT (demo)
      const nft = mintNftDemo(seriesNumber, companyName);
      logStep("NFT minted (demo)", nft);
      ...
      try {
        const funcUrl = `${supabaseUrl}/functions/v1/generate-formation-docs`;
        await fetch(funcUrl, {
          method: "POST",
          headers: {
      ...
      logStep("Formation complete via webhook", { companyId: company.id });
    } catch (err) {
      logStep("ERROR during company creation", { error: String(err) });
